...
with st.sidebar:
    ...
    st.markdown("---")
    st.info("💡 팁: 정보를 많이 추가할수록 퀴즈가 재미있어져요!")
...
# 메뉴별 화면
    ...
    with col1:
        st.metric("📊 저장된 정보", f"{total_items}개")
    with col2:
        category_count = sum(1 for cat in st.session_state.data.values() if cat)
        st.metric("📁 사용중인 카테고리", f"{category_count}개")
